...
**Goal:** Criar perfis de acesso do aplicativo no Strapi para admin e professor, sem promover nenhum e-mail hardcoded.
...
```ts
expect(buildAccessControlPlan('admin@example.com').roles).toEqual([
...
Call `ensureAppAccessControl(strapi, resolveAppAdminEmail(process.env.APP_ADMIN_EMAIL))` after auth plugin store synchronization. Do not use a hardcoded fallback.

- [x] **Step 2: Add idempotent database sync**

Ensure role creation/update, exact permission synchronization, and assignment to role `Admin` only when `APP_ADMIN_EMAIL` is explicitly configured.
...
Expected: `Admin` and `Teacher` roles exist, admin has read permissions for current content-types and quiz management, teacher has only quiz create, and no user is promoted unless `APP_ADMIN_EMAIL` is explicitly configured.
