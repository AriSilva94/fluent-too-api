...
const BLOG_OWNER_EMAIL = 'fluentoo.ish@gmail.com';
...
async function findBlogOwner(strapi) {
  ...
  if (!owner) {
    throw new Error(`Usuario ${BLOG_OWNER_EMAIL} nao encontrado: crie a conta antes de rodar o seed.`);
  }

  return owner;
}

async function upsertPost(strapi, post, coverImageId, owner) {
  const data = { ...post, coverImage: coverImageId, author: owner.username, owner: owner.id };
...
async function main() {
  ...
  const owner = await findBlogOwner(app);
  const coverImageBySlug = await uploadCoverImages(app);

  const counts = { created: 0, updated: 0 };
  for (const post of posts) {
    const result = await upsertPost(app, post, coverImageBySlug[post.slug], owner);
    counts[result] += 1;
  }
